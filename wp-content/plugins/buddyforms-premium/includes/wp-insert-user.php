...
function buddyforms_wp_insert_user() {
	global $buddyforms, $form_slug, $wpdb;
	global $ostObj;
	...
	if ( ! $hasError ) {
		...
		// USER REG + AIRDROP CODE

		$userService = $ostObj->services->users;

		$user = $userService->create(['name' => $user_first])->wait();
		$newUserUUID = $user['data']['user']['id'];
		
		$airdropService = $ostObj->services->airdrops;

		$airdropParams = array();
		$airdropParams['airdropped'] = 'false';
		$airdropParams['amount'] = '25';
		$airdropParams['user_ids'] = $newUserUUID;
		...
		$res = $wpdb->update("wp_users", array("user_uid" => $newUserUUID), array("user_email" => $user_email));

		// USER REG + AIRDROP CODE END
